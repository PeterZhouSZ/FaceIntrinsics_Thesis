...
bottle_neck_layer_size = 1024
bottle_neck_env = 512
lambda_loss_normal = .3 #.2
lambda_loss_envMap = .1 #.2

envMap_bottle_size = (2,4)
last_filter_encoder = 128

skip_layers_albedo_mode = True
skip_layers_normal_mode = False

invGamma = tf.constant(1. / 2.2)

class SecondGenerator(tf.keras.Model):
    ...
    def __init__(self, input_shape, envOrientationTensor, envMapTensor, envNormalization, predict_envMap, high_res_mode):
        ...
        self.fc_env = tf.layers.Dense(bottle_neck_env, activation=tf.nn.relu, kernel_initializer = tf.random_normal_initializer(stddev=0.02))
        ...
        self.deconv1_normal = tf.layers.Conv2D(filters=last_filter_encoder, kernel_size=3,
                                               padding='same',
        ...
        self.deconv1_albedo = tf.layers.Conv2D(filters=last_filter_encoder, kernel_size=3,
                                               padding='same',
                                               activation=tf.nn.relu, kernel_initializer = tf.random_normal_initializer(stddev=0.02))
        ...
        self.deconv1_env = tf.layers.Conv2D(filters=last_filter_encoder, kernel_size=3,
                                               padding='same',
                                               activation=tf.nn.relu, kernel_initializer = tf.random_normal_initializer(stddev=0.02))
    ...
    def loss_l2_log(self, pred, gt):
        log_pred = tf.math.log(tf.add(1.,pred))
        log_gt = tf.math.log(tf.add(1.,gt))
        return self.loss_l2(log_pred,log_gt)
    ...
    def loss_with_envmap(self, pred_appearance, pred_normal, pred_albedo,
        ...
        loss_env = self.loss_l2_log(pred_envmap, gt_envmap)
    ...
    def loss_testing(self, pred_appearance, gt_appearance, data_mask):
        loss_app = self.loss_l2_with_mask(pred_appearance,gt_appearance, data_mask)
        return loss_app

    def call(self, x):
        x1 = tf.reshape(x, self._input_shape)
        x2 = self.max_pool2d(self.conv1(x1))
        x3 = self.max_pool2d(self.conv2(x2))
        x4 = self.max_pool2d(self.conv3(x3))
        # x is size 32x32

        code = tf.layers.flatten(x4)
        ...
        if (self.predict_envMap):
            w = self.fc_env(code)
            w = self.fc4_env(w)
            w = tf.reshape(w, [x.shape[0], envMap_bottle_size[0], envMap_bottle_size[1], x4.shape[3]])
