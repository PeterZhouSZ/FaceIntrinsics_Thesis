...
class FirstDiscriminator(tf.keras.Model):
    def __init__(self):
        super(FirstDiscriminator, self).__init__()
        ...
        self.conv2 = tf.layers.Conv2D(filters=32, kernel_size=3,
                                      padding='same', #strides = 2,
                                      activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        self.conv3 = tf.layers.Conv2D(filters=64, kernel_size=3,
                                      padding='same', #strides = 2,
                                      activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        self.conv4 = tf.layers.Conv2D(filters=128, kernel_size=3,
                                      padding='same', #strides = 2,
                                      activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        self.conv5 = tf.layers.Conv2D(filters=256, kernel_size=4,
                                      padding='same',  strides = 2,
                                      activation=tf.nn.leaky_relu,
    ...
    def call(self, x):
        ...
        x = self.max_pool2d(self.conv4(x))
